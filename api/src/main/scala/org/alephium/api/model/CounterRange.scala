...
object CounterRange {
  ...
  val validator: Validator[CounterRange] = Validator.custom { counterRange =>
    if (counterRange.start < 0) {
      ValidationResult.Invalid(Some(s"`start` must not be negative"))
    } else {
      counterRange.endOpt match {
        case Some(end) =>
          if (end <= counterRange.start) {
            ValidationResult.Invalid(Some(s"`end` must be larger than `start`"))
          } else if (end - counterRange.start > MaxCounterRange) {
            ValidationResult.Invalid(
              Some(s"`end` must be smaller than ${counterRange.start + MaxCounterRange}")
            )
          } else {
        ...
        case None =>
          if (counterRange.start > Int.MaxValue - MaxCounterRange) {
            ValidationResult.Invalid(
              Some(s"`start` must be smaller than ${Int.MaxValue - MaxCounterRange}")
            )
          } else {
            ValidationResult.Valid
          }
