...
object UTXO {
  def from(ref: model.TxOutputRef, output: model.TxOutput): UTXO = {
    ...
    val (lockTime, additionalData) = output match {
      case o: model.AssetOutput    => Some(o.lockTime) -> Some(o.additionalData)
      case _: model.ContractOutput => None             -> None
    }
