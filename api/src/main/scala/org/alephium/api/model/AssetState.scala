...
final case class AddressAssetState(address: Address, assetState: AssetState)

object AddressAssetState {
  def from(assetOutput: model.AssetOutput): AddressAssetState = {
    AddressAssetState(Address.Asset(assetOutput.lockupScript), AssetState.from(assetOutput))
  }

  def from(output: model.TxOutput): AddressAssetState = {
    AddressAssetState(Address.from(output.lockupScript), AssetState.from(output))
  }
}
