...
class ApiModelSpec extends AlephiumSpec with ApiModelCodec with EitherValues with NumericHelpers {
  ...
  it should "encode/decode NodeInfo" in {
    val nodeInfo =
      NodeInfo(ReleaseVersion(0, 0, 0), isMining = true, NodeInfo.BuildInfo("1.2.3", "07b7f3e044"))
    val jsonRaw =
      s"""{"version":"v0.0.0","isMining":true,"buildInfo":{"releaseVersion":"1.2.3","commit":"07b7f3e044"}}"""
