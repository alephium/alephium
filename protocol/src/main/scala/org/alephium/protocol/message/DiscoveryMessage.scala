...
object DiscoveryMessage {
  ...
  final case class Header(version: Int, publicKey: ALFPublicKey, cliqueId: CliqueId)
  object Header {
    private val serde = Serde.tuple3[Int, ALFPublicKey, CliqueId]
  ...
  def serialize(message: DiscoveryMessage)(implicit config: DiscoveryConfig): ByteString = {
    ...
    val signature    = ALFSignatureSchema.sign(payloadBytes, config.discoveryPrivateKey)
  ...
  def deserialize(myCliqueId: CliqueId, input: ByteString)(
    ...
    for {
      ...
      rest1  = headerPair._2
      signaturePair <- serdeImpl[ALFSignature]._deserialize(rest1)
      signature = signaturePair._1
      rest2     = signaturePair._2
      payload <- Payload.deserialize(rest2).flatMap { payload =>
        if (ALFSignatureSchema.verify(rest2, signature, header.publicKey)) Right(payload)
