...
trait ContextGenerators extends MockFactory with NoIndexModelGenerators {
  def prepareContract(contract: StatefulContract,
    ...
    val worldState =
      mockWorldState.addContract(contractOutputRef, contractOutput, fields).toOption.get
    val obj     = contract.toObject(contractOutputRef.key, fields)
    val context = StatefulContext(ALF.Hash.zero, worldState)
    obj -> context
  }
}
