...
class ValidationSpec extends AlephiumFlowSpec {
  ...
  it should "validate group for block" in {
    forAll(ModelGen.blockGenFor(config.brokerInfo)) { block =>
      check(Validation.validateGroup(block))
    }
    forAll(ModelGen.blockGenNotFor(config.brokerInfo)) { block =>
      check(Validation.validateGroup(block), InvalidGroup)
    }
  }
  ...
  it should "validate timestamp for block" in {
    ...
    check(Validation.validateTimeStamp(header0, isSyncing = false))
    check(Validation.validateTimeStamp(header1, isSyncing = false), InvalidTimeStamp)
    check(Validation.validateTimeStamp(header2, isSyncing = false), InvalidTimeStamp)
    check(Validation.validateTimeStamp(header0, isSyncing = true))
    check(Validation.validateTimeStamp(header1, isSyncing = true))
    check(Validation.validateTimeStamp(header2, isSyncing = true), InvalidTimeStamp)
  }
  ...
  it should "validate nonEmpty transaction list" in {
    ...
    check(Validation.validateNonEmptyTransactions(block0))
    check(Validation.validateNonEmptyTransactions(block1), EmptyTransactionList)
  }
