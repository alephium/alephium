...
class BlockFlowSpec extends AlephiumSpec {
  ...
  it should "compute cached blocks" in new FlowFixture {
    ...
    cache1.contains(newBlocks(0).hash) is false
  ...
  trait InterGroupFixture extends FlowFixture { Test =>
    def hardFork: HardFork = HardFork.Rhone
  ...
  it should "transfer token for inter-group transactions: rhone" in new InterGroupFixture {
  ...
  it should "transfer token for inter-group transactions: danube" in new InterGroupFixture {
    override def hardFork: HardFork = HardFork.Danube
    ...
    blockFlow.genesisBlocks.foreachWithIndex { case (blocks, from) =>
      blocks.foreachWithIndex { case (block, to) =>
        block.transactions.foreachWithIndex { case (tx, index) =>
          from is to
          blockFlow.getTxConfirmationStatus(tx.id, ChainIndex.unsafe(from, to)) isE
  ...
  it should "return correct confirmations for intra group txs" in new FlowFixture {
    ...
    for {
      ...
      blockFlow0.getTxConfirmationStatus(block.transactions.head.id, chainIndex) isE None

      addAndCheck(blockFlow0, block)
      blockFlow0.getTxConfirmationStatus(block.transactions.head.id, chainIndex) isE
  ...
  it should "return correct confirmations for inter group txs" in new FlowFixture {
    ...
    for {
      ...
      blockFlow0.getTxConfirmationStatus(block0.transactions.head.id, chainIndex) isE None

      addAndCheck(blockFlow0, block0)
      blockFlow0.getTxConfirmationStatus(block0.transactions.head.id, chainIndex) isE
