...
class ServerTest extends AlephiumSpec {
  ...
  it should "boot and sync two cliques" in new Fixture("2-cliques-of-2-nodses") {
    ...
    request[FetchResponse](blockflowFetch(fromTs, toTs), rpcPort(masterPortClique1)).blocks.size is request[
      FetchResponse](blockflowFetch(fromTs, toTs), rpcPort(masterPortClique2)).blocks.size
  }
