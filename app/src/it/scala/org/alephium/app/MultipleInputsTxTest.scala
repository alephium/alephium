...
class MultipleInputsTxTest extends AlephiumActorSpec {
  ...
  it should "let only one address pay gas" in new CliqueFixture {
    val clique = bootClique(nbOfNodes = 1)
    clique.start()
    ...
    val utxos4      = currentUTXOs(address4).utxos
    val amountInit4 = utxos4.map(_.amount.value).fold(U256.Zero)(_ addUnsafe _)
    val amount4     = amountInit4.divUnsafe(U256.Two)
    ...
    val inputs: AVector[BuildMultiAddressesTransaction.Source] = AVector(
      BuildMultiAddressesTransaction
        .Source(pubKey(publicKey).bytes, destinations(amount), gasAmount = None),
      BuildMultiAddressesTransaction
        .Source(pubKey(publicKey2).bytes, destinations(amount2), gasAmount = Some(GasBox.zero)),
      BuildMultiAddressesTransaction
        .Source(
          pubKey(publicKey3).bytes,
          destinations(amount3, lockTime = Some(TimeStamp.now())),
          gasAmount = Some(GasBox.zero)
        ),
      BuildMultiAddressesTransaction.Source(
        pubKey(publicKey4).bytes,
        destinations(amount4, message = Some(ByteString.empty)),
    ...
    val amount4After =
      currentUTXOs(address4).utxos.map(_.amount.value).fold(U256.Zero)(_ addUnsafe _)

    amount4After is amountInit4.subUnsafe(amount4)
